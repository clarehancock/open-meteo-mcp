...
export function createServer() {
    ...
    server.tool("geocode_location", "Look up geographic coordinates (latitude/longitude) for a place name, e.g. " +
    ...
    server.tool("get_current_weather", "Get the current weather conditions (temperature, wind, precipitation, etc.) " +
        ...
        timezone: z
            .string()
            .default("auto")
            .describe("Timezone for returned timestamps, e.g. 'America/New_York' or 'auto' to infer from coordinates"),
    ...
    server.tool("get_weather_forecast", "Get a multi-day hourly and/or daily weather forecast (up to 16 days) for a " +
        ...
        timezone: z
            ...
            .default("auto")
            .describe("Timezone for returned timestamps, e.g. 'America/New_York' or 'auto' to infer from coordinates"),
    }, async ({ latitude, longitude, forecast_days, include_hourly, temperature_unit, wind_speed_unit, timezone, }) => {
        const url = new URL(FORECAST_BASE);
        url.searchParams.set("latitude", String(latitude));
        url.searchParams.set("longitude", String(longitude));
